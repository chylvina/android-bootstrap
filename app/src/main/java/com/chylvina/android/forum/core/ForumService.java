...
import static com.chylvina.android.forum.core.Constants.Http.*;
...
public class ForumService {
    ...
    public List<Topic> getTop10() throws IOException {
        try {
            HttpRequest request = execute(HttpRequest.get(URL_TOP10));
            TopicWrapper response = fromJson(request, TopicWrapper.class);
            if (response != null && response.results != null)
                return response.results;
            return Collections.emptyList();
        } catch (HttpRequestException e) {
